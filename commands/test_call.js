...
module.exports = {
  ...
  async execute(interaction, bot) {
    ...
    try {
      ...
      console.log(`[TESTE] Chamada de ${month} enviada. Mensagem ID: ${msg.id}`);

      const allMembers = await guild.members.fetch();
      const targetMembers = allMembers.filter(member => member.roles.cache.has(roleID) && !member.user.bot);
      ...
      await interaction.editReply(`✅ Chamada de teste enviada! ID da mensagem: ${msg.id}`);
