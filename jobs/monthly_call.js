        ...
        try {
            ...
            for (const [memberId, member] of targetMembers) {
                try {
                    ...
                    console.error(
                        `Não foi possível enviar DM para ${member.user.tag} (${member.id}):`,
                        error,
                    );
                }
            ...
            schedule.scheduleJob(dataRelatorio, async () => {
                try {
                    const relatorioChannel =
                        await guild.channels.fetch(reportChannelID);
                    const mensagemChamada = await channel.messages.fetch(
                        msg.id,
                    );
                    const reaction =
                        mensagemChamada.reactions.cache.get(emojiID);
                    ...
                    console.error(
                        "Erro ao gerar o relatório automático de 7 dias:",
                        err,
                    );
                }
            });
