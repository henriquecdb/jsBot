  ...
  while (attempt < 3) {
    try {
      ...
      if (err.name === 'GatewayRateLimitError' && err.data?.retry_after) {
        ...
        console.warn(`Rate limit ao buscar membros do guild. Aguardando ${waitMs}ms...`);
...
async function sendDMsToMembers(bot, guild, channelID, month, roleID) {
  const allMembers = guild.members.cache.size > 0 ? guild.members.cache : await safeFetchGuildMembers(guild);
...
function scheduleReport(bot, channel, msgID, emojiID, reportChannelID, month, roleID, startDate = new Date()) {
...
async function generateReport(bot, channel, msgID, emojiID, reportChannelID, month, roleID) {
  try {
    ...
    let reaction = mensagemChamada.reactions.cache.find(
      (r) => r.emoji.id === emojiID || r.emoji.name === emojiID || r.emoji.identifier === emojiID,
    );
    ...
    if (!reaction) {
      console.error(`Emoji de reação ${emojiID} não encontrado na mensagem ${msgID}.`);
      return;
    }
